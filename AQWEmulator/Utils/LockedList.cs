...
namespace AQWEmulator.Utils
{
    public class LockedList<T> : List<T>
    {
        ...
        public LockedList(IEnumerable<T> otherlist)
            : base(otherlist)
        {
        }
        ...
        {
            lock (_lock) base.InsertRange(index, collection);
        }
        ...
        {
            lock (_lock) return new List<T>(this);
        }

        public new T Find(Predicate<T> match)
        {
            lock (_lock) return base.Find(match);
        }
